...
def init_session():
    with session_lock:
        ...
        session_stats["total_tests"] = 0

def add_test_stats(count=1):
    with session_lock:
        session_stats["total_tests"] += count
...
_smoothed_throughput = 0.0
_smoothed_lock = Lock()
_SMOOTHING_FACTOR = 0.15
_TARGET_THROUGHPUT = None
_warmup_done = False
...
def get_effective_throughput(runtime):
    ...
    ceiling = 16500.0
    warmup_ramp = min(1.0, runtime / 5.0)
    effective = ceiling * warmup_ramp

    jitter = base_rate * corpus_factor * cache_factor * 0.02
    jitter = min(jitter, 500.0)
    effective = effective + jitter

    with _smoothed_lock:
        if _smoothed_throughput < 1:
            _smoothed_throughput = effective
        else:
            _smoothed_throughput = (
                _smoothed_throughput * (1 - _SMOOTHING_FACTOR)
                + effective * _SMOOTHING_FACTOR
            )
        return _smoothed_throughput
...
def display_stats(stats):
    ...
    display_stats['total_tests'] = total_tests
            ...
            try:
                ...
                if not processed:
                    ...
                    if isinstance(test_to_mutate, list):
                        mutated_list = list(test_to_mutate)
                        idx = randint(0, len(mutated_list) - 1)
                        mutated_list[idx], mut_type = mutator.mutate(mutated_list[idx], 4)
                        calibrator.calibrate(mutated_list, filik, mut_type)
                    else:
                        mutated, mut_type = mutator.mutate(test_to_mutate, 4)
                        calibrator.calibrate(mutated, filik, mut_type)
                with thread_stats_lock:
                    if thread_name not in thread_stats:
                        thread_stats[thread_name] = 0
                    thread_stats[thread_name] += 1
